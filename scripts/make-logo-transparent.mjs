#!/usr/bin/env node
// ロゴ画像（白背景PNG）を背景透過＋余白トリミングして public/logo.png に出力する。
// 使い方: node scripts/make-logo-transparent.mjs <入力PNGのパス>
//   例:   node scripts/make-logo-transparent.mjs assets/logo-original.png
...
const input = process.argv[2] ?? "assets/logo-original.png";
const output = "public/logo.png";

let png;
try {
  png = PNG.sync.read(readFileSync(input));
} catch (e) {
